...
public class RankingGoldActivity extends BaseActivity {
        ...
        public void handleMessage(Message msg) {
            ...
            switch (msg.what) {
                ...
                case Constants.CODE_EXECUTE_SUCCESS:
                    //toastMsg = "数据请求成功！";
                    pageNum = 1;
                    recycleAdapter = new RankingAdapter(RankingGoldActivity.this, rankList, flag, sortFlag);
            ...
            if (toastMsg != null) {
                Toast.makeText(RankingGoldActivity.this, toastMsg, Toast.LENGTH_SHORT).show();
            }
...
//        userId = getIntent().getStringExtra(Constants.EXTRA_USER_ID);
    ...
    public void onCreateCustomToolBar(Toolbar toolbar) {
        ...
        tool_bar_right_bottom_button.setText(R.string.flower_market_flowers);
        tool_bar_right_bottom_button.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                Intent RankingGoldIntent = new Intent();
                RankingGoldIntent.setClass(RankingGoldActivity.this, RankingFlowerActivity.class);
                startActivity(RankingGoldIntent);
                recycleAdapter.clear();
                rankList.clear();
                finish();
            }
    ...
    private void initPullRefresh() {
        swipeRefreshLayout.setOnRefreshListener(new SwipeRefreshLayout.OnRefreshListener() {
            ...
            public void onRefresh() {
                if (Utils.isConnect(RankingGoldActivity.this)) {
                    ...
                    Utils.showToast(RankingGoldActivity.this, R.string.netWrong);
                }
            }
        });
    }
    ...
    private int updateData(int pageNumber) {
        ...
        try {
            ...
            // 获取响应的结果信息
            if (spinner_flag == 1) {
                json = getPostData(Constants.URL_RANKING_ALLGOLD, params);
            } else {
                json = getPostData(Constants.URL_RANKING_GOLD, params);
            }
            ...
            if (json != null) {
                ...
                Log.d(TAG, "===RankingGoldActivity updateData() jsonObject = " + jsonObject.toString());
                result = jsonObject.getBoolean("success");
                if (result) {
                    JSONObject dataObject = jsonObject.getJSONObject("data");
                    Log.d(TAG, "===RankingGoldActivity updateData() dataObject = " + dataObject.toString());
                    rankingBean = null;
                    if (pageNumber == 1) {
                        taskListArray = dataObject.getJSONArray("sort");
                        Log.d(TAG, "===RankingGoldActivity updateData() taskListArray = " + taskListArray.toString());
                        ...
                        for (int i = 0; i < listLength; i++) {
                            ...
                            Log.d(TAG, "===RankingGoldActivity updateData() rankData = " + rankData.toString());
                            ...
                            if (rankingBean.getHeadPicUrl() != null && !"".equals(rankingBean.getHeadPicUrl().toString())) {
                                // rankingBean.setHeadPicBitmap(Utils.getBitmap(Constants.PICTURE_BASE_URL + rankingBean.getHeadPicUrl().toString()));
/*                                Bitmap logoBitmap = Utils.getBitmap(Constants.PICTURE_BASE_URL + rankData.get("headPicUrl").toString());
                    ...
                    if (spinner_flag == 1) {
                        taskListArr = dataObject.getJSONArray("goldAllRankList");
                    } else {
                        taskListArr = dataObject.getJSONArray("goldRankList");
                    }
                    Log.d(TAG, "===RankingGoldActivity updateData() taskListArr = " + taskListArr.toString());
